	...
	for key, value in ipairs(resources) do
		...
		if resourceName == "global" or resourceName == "mysql" or resourceNmae == "pool" then
			if getResourceState(value) == "loaded" or getResourceState(value) == "stopping" or getResourceState(value) == "failed to load" then
				...
				outputChatBox("       - The World Life Roleplay Administration Team", thePlayer, 255, 0, 0)
				break
			end
		end
	end
...
addEventHandler("accounts:login:attempt", getRootElement(), 
	function (username, password, wantsLoginHash)
		...
		if (string.len(cpypassword) ~= 64) then
			...
			accountCheckQueryStr = "SELECT `id`, `username`, `banned`, `appstate`, `admin`,  `warn_style`, `adminduty`, `adminreports`, `hiddenadmin`, `adminjail`, `adminjail_time`, `adminjail_by`, `adminjail_reason`, `monitored`, `email` FROM `accounts` WHERE `username`='" .. safeusername .. "' AND `password`='" .. safepassword .. "'"
		else
			safeusername = mysql:escape_string(username)
			safepassword = mysql:escape_string(password)
			accountCheckQueryStr = "SELECT `id`, `username`, `banned`, `appstate`, `admin`, `warn_style`, `adminduty`, `adminreports`, `hiddenadmin`, `adminjail`, `adminjail_time`, `adminjail_by`, `adminjail_reason`, `monitored`, `email` FROM `accounts` WHERE `username`='" .. safeusername .. "' AND `loginhash`='" .. safepassword .. "'"
		end	
