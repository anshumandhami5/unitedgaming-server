...
function infoDisplay(thePlayer)
	if (exports.global:isPlayerAdmin(thePlayer)) then
		...
		outputChatBox("---[ Server: mta.unitedgaming.org Port: 22003", getRootElement(), 255, 194, 15)
		outputChatBox("---[ Ventrilo: vent.unitedgaming.org Port: 3083", getRootElement(), 255, 194, 15)
		outputChatBox("---[ UCP: www.unitedgaming.org", getRootElement(), 255, 194, 15)
		outputChatBox("---[ Forums: www.unitedgaming.org/forums", getRootElement(), 255, 194, 15)
		outputChatBox("---[ Mantis: www.unitedgaming.org/mantis", getRootElement(), 255, 194, 15)
	end
end
addCommandHandler("uginfo", infoDisplay)
...
--[[function makePlayerAdmin(thePlayer, commandName, who, rank)
	if exports.global:isPlayerScripter(thePlayer) or exports.global:isPlayerHeadAdmin(thePlayer) or exports.global:isPlayerGMTeamLeader(thePlayer) then
...
function makePlayerAdmin(thePlayer, commandName, who, rank) --/ MAXIME
	if exports.global:isPlayerScripter(thePlayer) or exports.global:isPlayerHeadAdmin(thePlayer) or exports.global:isPlayerGMTeamLeader(thePlayer) or exports.global:isPlayerHeadGameMaster(thePlayer) then
		if not (who) or not (tonumber(rank)) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Player Partial Name/ID] [Rank, -1 .. -4 = GMs, 1 .. 7 = Admins]", thePlayer, 255, 194, 14)
	...
	if (exports.global:isPlayerAdmin(thePlayer)) then
		if not (targetPlayer) or not (...) then
			...
			if targetPlayer then
				...
				mysql:query_free("UPDATE accounts SET warns=" .. mysql:escape_string(warns) .. ", monitored = 'Was warned for " .. mysql:escape_string(reason) .. "' WHERE id = " .. mysql:escape_string(accountID) )
...
function earthquake(thePlayer, commandName)
	if (exports.global:isPlayerAdmin(thePlayer) and exports.global:isPlayerScripter(thePlayer)) then
		local players = exports.pool:getPoolElementsByType("player")			
		for index, arrayPlayer in ipairs(players) do
