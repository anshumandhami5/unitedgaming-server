...
function maximeReportsReminder()
	for key, value in ipairs(getElementsByType("player")) do
		local level = getElementData( value, "adminlevel" ) or 0
		local aod = getElementData( value, "adminduty" ) or 0
		local god = getElementData( value, "account:gmduty" )
		if level < 0 and god ~= 0 then --GM onduty
			showUnansweredReports(value)
		elseif level > 0 and aod ~= 0 then -- Admin onduty
			showUnansweredReports(value)
		end
	end
end
...
function compareTimeString(time1, time2) -- return 0 = equal; 1 = greater; -1 = smaller
	if time1 and time2 then
		if tonumber(time1:sub(1,2)) == tonumber(time2:sub(1,2)) then 
			if tonumber(time1:sub(4,5)) == tonumber(time1:sub(4,5)) then
				return 0 
			elseif tonumber(time1:sub(4,5)) > tonumber(time1:sub(4,5)) then
				return 1
			elseif tonumber(time1:sub(4,5)) < tonumber(time1:sub(4,5)) then
				return 0
			end
		elseif tonumber(time1:sub(1,2)) > tonumber(time2:sub(1,2)) then
			return 1
		elseif tonumber(time1:sub(1,2)) < tonumber(time2:sub(1,2)) then
			return -1
		end
...
function sortReportsByTime(reports)
	local oldestReport = nil
	local reportPool = {}
	for i = 1, 50 do
		for j = 1, 50 do 
			if reports[i] and reports[j] then 
				if compareTimeString(reports[i][4], reports[j][4]) > 0 then
					oldestReport = reports[j][4]
				else
					oldestReport = reports[i][4]
				end
			end
		end
		reportPool[i] = oldestReport
	end
	return reportPool
end
		...
		if (isElement(theAdmin)) then
			...
			if isGMreport then
				for key, value in ipairs(exports.global:getAdmins()) do
					...
					if adminduty == 1 then
						outputChatBox(" [#" .. report .."] Player " .. getPlayerName(source) .. " left the game.", value, 255, 126, 0)--200, 240, 120)
						update = true
					end
				end
			else
				...
				if (reportHandler) then
					...
					if isGMreport then
						...
						for key, value in ipairs(admins) do
							local adminduty = getElementData(value, "account:gmduty")
							if (adminduty== true) then
								outputChatBox(" [GM #" .. id .. "] - " .. getPlayerName(thePlayer) .. " has marked report #" .. id .. " as false. -", value, 70, 200, 30)
							end
						end		
						outputChatBox("[" .. timestring .. "] Your report (#" .. id .. ") was marked as false by " .. getPlayerName(thePlayer) .. ".", reportingPlayer, 70, 200, 30)
					else
						...
						for key, value in ipairs(admins) do
							local adminduty = getElementData(value, "adminduty")
							if (adminduty==1) then
								outputChatBox(" [#" .. id .. "] - " .. getPlayerName(thePlayer) .. " ("..adminNAme..") has marked report #" .. id .. " as false. -", value, 255, 126, 0)--200, 240, 120)
							end
						end
						
						outputChatBox("[" .. timestring .. "] Your report (#" .. id .. ") was marked as false by " .. getPlayerName(thePlayer) .. " ("..adminNAme..").", reportingPlayer, 255, 126, 0)--200, 240, 120)
					end
		...
		if not (id) then
			...
			if not (reports[id]) then
				...
				if (reportHandler) then
					...
					if reportingPlayer == thePlayer then
						outputChatBox("You can not accept your own report.",thePlayer, 255,0,0)
						return false
					elseif reportedPlayer == thePlayer then
						outputChatBox("You better let someone else to handler this report because it's agaisnt you.",thePlayer, 255,0,0)
						return false
					end
		...
		if not (id) then
			...
			elseif (reports[id][5] ~= thePlayer) then
				...
				if (isElement(reporter)) then
					...
					exports['anticheat-system']:changeProtectedElementDataEx(reporter, "gmreport", false, true)
					exports['anticheat-system']:changeProtectedElementDataEx(reporter, "reportadmin", false, false)
					outputChatBox(adminTitle.." "..getPlayerName(thePlayer) .. " ("..adminName..") has closed your report.", reporter, 210, 105, 50)
				end
