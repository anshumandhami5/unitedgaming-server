...
function loadOneVehicle(id, hasCoroutine, loadDeletedOne)
	...
	if row then
		...
		if veh then
			...
			--suspensionLowerLimit by Anthony
			local limit = row["suspensionLowerLimit"]
			if limit ~= nil then
				setVehicleHandling(veh, "suspensionLowerLimit", tonumber(limit) or nil)
			end
			
			--driveType by Anthony
			local dt = row["driveType"]
			if dt ~= nil then
				setVehicleHandling(veh, "driveType", tostring(dt) or nil)
			end
